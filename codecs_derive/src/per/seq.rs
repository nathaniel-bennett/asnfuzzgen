...
pub(super) fn generate_aper_codec_for_asn_sequence(
    ...
    let (codec_path, codec_encode_fn, codec_decode_fn, ty_encode_path, ty_decode_path) = if aligned
    {
        (
            ...
            quote!(aper_encode),
            quote!(aper_decode),
            quote!(asnfuzzgen_codecs::aper::encode::encode_sequence_header),
            quote!(asnfuzzgen_codecs::aper::decode::decode_sequence_header),
        )
    } else {
        (
            ...
            quote!(uper_encode),
            quote!(uper_decode),
            quote!(asnfuzzgen_codecs::uper::encode::encode_sequence_header),
            quote!(asnfuzzgen_codecs::uper::decode::decode_sequence_header),
        )
    };
