...
pub(super) fn generate_aper_codec_for_asn_charstring(
    ...
    let (codec_path, codec_encode_fn, codec_decode_fn, ty_encode_path, ty_decode_path) = if aligned
    {
        (
            ...
            quote!(aper_encode),
            quote!(aper_decode),
            quote!(asnfuzzgen_codecs::aper::encode::#encode_fn_name),
            quote!(asnfuzzgen_codecs::aper::decode::#decode_fn_name),
        )
    } else {
        (
            ...
            quote!(uper_encode),
            quote!(uper_decode),
            quote!(asnfuzzgen_codecs::uper::encode::#encode_fn_name),
            quote!(asnfuzzgen_codecs::uper::decode::#decode_fn_name),
        )
    };
    let name = &ast.ident;
