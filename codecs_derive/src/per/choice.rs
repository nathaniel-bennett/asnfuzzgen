...
pub(super) fn generate_aper_codec_for_asn_choice(
    ...
    let (codec_path, codec_encode_fn, codec_decode_fn, ty_encode_path, ty_decode_path) = if aligned
    {
        (
            ...
            quote!(aper_encode),
            quote!(aper_decode),
            quote!(asnfuzzgen_codecs::aper::encode::encode_choice_idx),
            quote!(asnfuzzgen_codecs::aper::decode::decode_choice_idx),
        )
    } else {
        (
            ...
            quote!(uper_encode),
            quote!(uper_decode),
            quote!(asnfuzzgen_codecs::uper::encode::encode_choice_idx),
            quote!(asnfuzzgen_codecs::uper::decode::decode_choice_idx),
        )
    };
            ...
            fn #codec_decode_fn(data: &mut asnfuzzgen_codecs::PerCodecData) -> Result<Self::Output, asnfuzzgen_codecs::PerCodecError> {
                ...
                if !extended {
                    match idx {
                        ...
                        _ => Err(asnfuzzgen_codecs::PerCodecError::new(format!("Index {} is not a valid Choice Index", idx).as_str()))
                    }
                } else {
                    Err(asnfuzzgen_codecs::PerCodecError::new("CHOICE Additions not supported yet."))
                }
            }
