...
pub(super) fn generate_aper_codec_for_asn_enumerated(
    ...
    let (codec_path, codec_encode_fn, codec_decode_fn, ty_encode_path, ty_decode_path) = if aligned
    {
        (
            ...
            quote!(aper_encode),
            quote!(aper_decode),
            quote!(asnfuzzgen_codecs::aper::encode::encode_enumerated),
            quote!(asnfuzzgen_codecs::aper::decode::decode_enumerated),
        )
    } else {
        (
            ...
            quote!(uper_encode),
            quote!(uper_decode),
            quote!(asnfuzzgen_codecs::uper::encode::encode_enumerated),
            quote!(asnfuzzgen_codecs::uper::decode::decode_enumerated),
        )
    };
