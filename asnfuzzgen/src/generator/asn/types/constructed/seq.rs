        ...
        } = self
        {
            ...
            for (idx, c) in components.iter().enumerate() {
                ...
                if c.key_field {
                    ...
                    from_entropy_defs.extend(
                        quote! { let #comp_field_ident: #ty_ident = source.get_entropic()?; },
                    );
            ...
            if let (Some((comp_field_ident, ty_ident)), Some(key_value_field_ident)) =
                (key_field, key_value_name)
            {
