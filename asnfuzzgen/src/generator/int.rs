...
lazy_static! {
    static ref CODEC_TOKENS: HashMap<Codec, String> = {
        ...
        m.insert(
            Codec::Aper,
            "asnfuzzgen_codecs_derive::AperCodec".to_string(),
        );
        m.insert(
            Codec::Uper,
            "asnfuzzgen_codecs_derive::UperCodec".to_string(),
        );
        m
    };
...
impl Generator {
    pub(crate) fn new(visibility: &Visibility, codec: Codec, derives: Vec<Derive>) -> Self {
        Generator {
            items: vec![],
            counter: 1,
            aux_items: vec![],
    ...
    pub(crate) fn generate(&mut self, resolver: &Resolver) -> Result<String, Error> {
        ...
        items.push(quote! {
            ...
            use asnfuzzgen_codecs::Asn1Choice;
    ...
    pub(crate) fn generate_derive_tokens(&self, entropic: bool) -> TokenStream {
        let mut tokens = vec![];
        let codec_token = CODEC_TOKENS.get(&self.codec).unwrap();
